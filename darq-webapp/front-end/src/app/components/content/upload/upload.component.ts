...
export class UploadComponent implements OnInit {
	...
	constructor(private http: Http, private router : Router, private notifierService: NotifierService, private dialog : MatDialog) {
	...
	create() {
		...
		return this.http.post('api/adf/upload', form).toPromise().then(function (x) {
				...
				ctrl.notifierService.notify("success","Aggregate Data File Uploaded Successfully");
				ctrl.router.navigate(["/data/files"]).then(function (x) {
				});
			},
			function (x) {
				ctrl.blockUIList.stop();
				ctrl.notifierService.notify("error","Could not upload Aggregate Data File due to : "+x.error);
				ctrl.router.navigate(["/data/files"]).then(function (x) {
				});
			});
	}
