...
export class AnalysisPayloadDialogComponent implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<AnalysisPayloadDialogComponent>,
    @Inject(MAT_DIALOG_DATA) public data: any,
    private $range: RangesService,
  ...
  removeField(i, list) {
    const e = list[i];
    for (const gF of this.groupFilters) {
      delete gF[e];
    }
    list.splice(i, 1);
  }
  ...
  reset() {
    this.types = [];
    for (const x in this.data.names) {
      this.types.push({
        key: x,
  ...
  submittable() {
    const f = this.filters.length > 0 || this.groupBy.length > 0;
    let v = true;
  ...
  resolve() {
    const payload: AnalysisPayload = new AnalysisPayload();
    ...
    for (const row of this.groupFilters) {
      const rerow: GroupFilter = { values: {}, threshold: row.threshold };

      for (const cell in row.values) {
  ...
  ngOnInit() {
    ...
    for (const d of this.configuration.detections) {
  ...
  canStep() {
    ...
    if (i === 1) {
      ...
      const must = (((CG[this.selected.key] === CG.VD) || (CG[this.selected.key] === CG.PD)) && this.selected.fields.lastIndexOf(Field.DETECTION) != -1) || (((CG[this.selected.key] === CG.VT) || (CG[this.selected.key] === CG.PT)) && this.selected.fields.lastIndexOf(Field.CODE) != -1);
  ...
  plural() {
    const p: AnalysisPayload = new AnalysisPayload();
  ...
  needOption() {
    ...
    p.groupFilters = this.groupFilters;
    p.groupBy = this.groupBy;
    return !this.$template.groupFilterActivated(p);
  }
