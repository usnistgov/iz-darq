...
public class ORExpression extends Expression {
	...
	public String getStringValue() {
		...
		return "(" + String.join(" OR ", expressionStrings) + ")";
	}
	...
	public List<String> validate() {
		...
		if(expressions == null || expressions.size() < 2) {
			errors.add("OR operator shall have at least two expressions");
		} else {
			expressions.forEach((expression) -> {
				if(expression == null) {
					errors.add("OR operator's operand expression shall not be null");
				} else {
