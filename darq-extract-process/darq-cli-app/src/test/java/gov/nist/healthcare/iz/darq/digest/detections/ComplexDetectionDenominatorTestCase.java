...
public class ComplexDetectionDenominatorTestCase {
	static TemporaryFolder folder = new TemporaryFolder();
	static SqliteADFReader reader;
	static ComplexDetectionDenominatorMock mock;
	...
	public static void setup() throws Exception {
		folder.create();
		mock = new ComplexDetectionDenominatorMock();
	...
	public void checkComplexDetections() throws Exception {
		...
		List<List<Integer>> rows = Arrays.asList(
				Arrays.asList(
						...
						d.findId(Field.DETECTION, ComplexDetectionDenominatorMock.DETECTION_CODE), 0, 2),
				Arrays.asList(
						d.findId(Field.PROVIDER, DigestUtils.md5DigestAsHex("ANY".getBytes())),
						d.findId(Field.AGE_GROUP, "0g"),
						d.findId(Field.DETECTION, Detection.VaccinationLotNumberIsMissing.getMqeMqeCode()), 0, 2),
				Arrays.asList(
						d.findId(Field.PROVIDER, DigestUtils.md5DigestAsHex("ANY".getBytes())),
						d.findId(Field.AGE_GROUP, "0g"),
