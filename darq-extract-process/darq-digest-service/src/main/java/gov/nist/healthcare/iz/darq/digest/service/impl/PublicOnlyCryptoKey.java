...
public class PublicOnlyCryptoKey implements CryptoKey {
    ...
    public void setPublicKeyFromResource() throws Exception {
        InputStream key = PublicOnlyCryptoKey.class.getResourceAsStream("/" + Constants.PUB_KEY_RESOURCE_NAME);
        if(key == null) {
            throw new Exception("Public Key in resource at location " + Constants.PUB_KEY_RESOURCE_NAME + " not found");
        } else {
    ...
    public void setPublicKeyFromLocation(String publicKeyLocation) throws Exception {
        if(!Strings.isNullOrEmpty(publicKeyLocation)) {
            ...
            if(path.toFile().exists()) {
                this.publicKey = getPublicKeyFromPEMStream(Files.newInputStream(path.toFile().toPath()));
            } else {
